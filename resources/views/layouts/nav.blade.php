<nav class="navbar navbar-expand-lg navbar-dark bg-success" style="min-height: 7vh;">
    <div class="container-fluid" style="width: 80%;">
        ...
        <a class="navbar-brand fw-bold" style="font-size: 30px;" href="{{ route('index') }}">Agrivall</a>
            ...
            <div class="d-flex flex-column flex-lg-row gap-2 align-items-lg-center ms-lg-3 mt-2 mt-lg-0">
                @auth
                    <span class="navbar-text text-white">
                        <a href="{{ route('profile.edit') }}">Hola, {{ auth()->user()->name }}</a>
                    </span>

                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button class="btn text-white btn-sm"
                            style="background-color: #735322; border-radius: 10px;">Salir</button>
                    </form>
                @else
                    <a class="btn text-white btn-sm" style="background-color: #735322; border-radius: 10px;"
